...
int solveSudoku(int row, int col, int matrix[MAX_SIZE][MAX_SIZE], int box_sz, int grid_sz, int depth_cutoff)
{
    ...
    {
        if (solveSudoku(row, col + 1, matrix, box_sz, grid_sz, depth_cutoff))
        {
    ...
    else
    {
        ...
        for (num = 1; num <= box_sz; num++)
        {
            ...
            {
            #pragma omp task firstprivate(num, col, row, matrix, depth_cutoff) if (depth_cutoff < MAX_DEPTH_CUTOFF)
                {
                    depth_cutoff = depth_cutoff + 1;

                    int privateMatrix[MAX_SIZE][MAX_SIZE];
                    memcpy(privateMatrix, matrix, sizeof(int) * MAX_SIZE * MAX_SIZE);
                    

                        privateMatrix[row][col] = num;

                        if (solveSudoku(row, col + 1, privateMatrix, box_sz, grid_sz, depth_cutoff))
                            printMatrix(privateMatrix, box_sz);

                        privateMatrix[row][col] = EMPTY;  
                }
            }
        }
#pragma omp taskwait
...
#pragma omp single
        {
            solveSudoku(0, 0, matrix, box_sz, grid_sz, depth_cutoff);
        }
    }
