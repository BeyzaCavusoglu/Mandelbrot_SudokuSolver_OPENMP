...
int solveSudoku(int row, int col, int matrix[MAX_SIZE][MAX_SIZE], int box_sz, int grid_sz, int *solution_found)
{
    if (col > (box_sz - 1))
    {
        col = 0;
    ...
    {
        if (solveSudoku(row, col + 1, matrix, box_sz, grid_sz, solution_found))
        {
    ...
    else
    {
        ...
        for (num = 1; num <= box_sz; num++)
        {
            ...
            {
            #pragma omp task firstprivate(num, col, row, matrix)
            {
             if (!(*solution_found))
                {
                    ...
                    if (solveSudoku(row, col + 1, privateMatrix, box_sz, grid_sz, solution_found))
                    {
...
#pragma omp single
        {
            solveSudoku(0, 0, matrix, box_sz, grid_sz, &solution_found);
        }
    }
